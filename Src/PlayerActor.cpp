...
void PlayerActor::Update(float deltaTime)
{
	...
	else if (position.y > groundHeight)
	{
		...
		//落下判定
		const bool isFloting = position.y > groundHeight + 0.1f;//地面から浮いているか
		if (!isInAir && isFloting && !boardingActor)
		{
			isInAir = true;
		}
		//重力を加える
		...
		if (attackTimer > 0.05f && attackTimer < 0.6f)
		{
			if (!attackCollision)
			{
				...
				const glm::vec3 front =
					glm::rotate(glm::mat4(1),rotation.y,glm::vec3(0,1,0)) * glm::vec4(0,0,1.5f,1);
				attackCollision =
					std::make_shared<Actor>(
						"PlayerAttackCollision", 5, position + front + glm::vec3(0, 1, 0),
						glm::vec3(0), glm::vec3(radian));
...
void PlayerActor::CheckRun(const GamePad& gamepad)
{
	...
	if (glm::dot(move, move))
	{
		...
		//物体に乗っているときは地形の勾配を考慮して移動方向を調整する
		if (!boardingActor)
		{
			//移動方向の地形の勾配(grandient)を計算
			const float minGrandient = glm::radians(-60.0f);//沿うことのできる勾配の最小値
			const float maxGrandient = glm::radians(60.0f);//沿うことのできる勾配の最大値
			const float frontY =
				heightMap->Height(position + move * 0.5f) - position.y - 0.01f;
			const float grandient =
				glm::clamp(std::atan2(frontY, 0.05f), minGrandient, maxGrandient);

			//地形に沿うように移動速度を設定
			const glm::vec3 axis = glm::normalize(glm::cross(move, glm::vec3(0, 1, 0)));
			move = glm::rotate(glm::mat4(1), grandient, axis) * glm::vec4(move, 1.0f);
		}
...
void PlayerActor::OnHit(const ActorPtr& b, const glm::vec3& p)
{
	...
	else
	{
		...
		const glm::vec3 deltaVelcity = velocity * deltaTime;
		position -= deltaVelcity;
		colWorld.s.center -= deltaVelcity;
	}
	SetBoardingActor(b);
}
