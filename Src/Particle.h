...
{
	int id = 0;//エミッタ―を識別するためのID
	glm::vec3 position = glm::vec3(0);//エミッタ―の位置
	glm::vec3 rotation = glm::vec3(0);//エミッタ―の向き
	float duration = 1.0f;//パーティクルを放出する変数
	...
	std::string imagePath;//テクスチャファイル名

	glm::ivec2 titles = glm::ivec2(1);//テクスチャの縦と横の分割数
...
public:
	...
	~ParticleSystem() = default;glm::vec3 acceleration = glm::vec3(0);//加速度
	glm::vec2 scale = glm::vec2(1);//大きさ
	float rotation = 0;//回転
	glm::vec4 color = glm::vec4(1);//	色と不透明度
