...
{
	//放出位置と速度を求めるための行列を計算
	const float rx = std::sqrt(RandamFloat(0,1));//X軸方向の放出位置を0～1の範囲で表した値
	const float ry = RandamFloat(0, glm::two_pi<float>());//0～360度の範囲でY軸回転させる値
...
bool ParticleSystem::Init(size_t maxParticleCount)
{
	...
	{
		std::cerr << "[エラー]パーティクルシステムの初期化に失敗\n";
		return false;
	}
	...
	{
		std::cerr << "[エラー]パーティクルシステムお初期化に失敗\n";
		return false;
	}
...
* @param id 検索するID
*
* @retval nullptr以外 引数idと一致するIDを持つエミッタ―
