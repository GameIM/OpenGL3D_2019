...
*/
	bool Window::Init(int w, int h, const char* title)
	{
		...
		std::cout << "SHDING_LANGUAGE_VERSION" << shading_language_version << std::endl;
...
*/
	...
	*/
	enum GAMEPAD_BUTTON
	{
		...
		GAMEPAD_BUTTON_L_THUMB,//左ボタン
		GAMEPAD_BUTTON_R_THUMB,//右ボタン
	...
	*/
	void Window::UpdateGamePad()
	{
		...
		if (axes && buttons && axesCount >= 2 && buttonCount >= 8)
		{
			...
			//配列インデックスとGamePadキー(コントローラー)の対応表
			static const struct {
				int dataIndex;
				uint32_t gamepadBit;
			} keyMap[] = {
