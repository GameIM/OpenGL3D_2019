...
bool SpriteRenderer::Init(size_t maxSpriteCount, const char* vsPath, const char* fsPath)
{
	...
	for (GLshort i = 0; i < maxSpriteCount; i++)
	{
		indices[i * 6 + 0] = (i * 4) + 0;
		indices[i * 6 + 1] = (i * 4) + 1;
		indices[i * 6 + 2] = (i * 4) + 2;
	...
	vao.VertexAttribPointer(1, 4, GL_FLOAT, GL_FALSE, sizeof(Vertex),
		offsetof(Vertex, color));
	vao.VertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(Vertex),
	...
	*/
	void SpriteRenderer::Draw(const glm::vec2& screenSize) const
	{
		...
		const glm::mat4x4 matProj = glm::ortho(
			-halfScreenSize.x, halfScreenSize.x, -halfScreenSize.y,
